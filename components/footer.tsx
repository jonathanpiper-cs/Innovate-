...
export default function Footer({ footer, entries }: { footer: FooterProps; entries: Entry }) {
	...
	function buildNavigation(ent: Entry, ft: FooterProps) {
		...
		if (ent.length !== newFooter.navigation.link.length) {
			ent.forEach((entry) => {
				const fFound = newFooter?.navigation.link.find((nlink: Links) => nlink.title === entry.title)
				if (!fFound) {
					...
					{footerData && footerData.logo ? (
						<Link href="/">
							<a className="logo-tag">
								<img
									src={footerData.logo.url.replace("images.contentstack.io", process.env.CDN_HOST as string)}
						...
						{footerData ? (
							footerData.social?.social_share.map((social) => (
								<a href={social.link.href} title={social.link.title} key={social.link.title}>
									{social.icon && (
										<img
											src={social.icon.url.replace("images.contentstack.io", process.env.CDN_HOST as string)}
